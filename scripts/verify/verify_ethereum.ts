 ...
 * verify_ethereum.ts
 *
 * Ethereum ağları için kontrat doğrulama (Etherscan).
 * Desteklenen ağlar: mainnet, sepolia
 *
 * Kullanım:
...
const SUPPORTED = ["mainnet", "sepolia"];
...
async function main() {
  if (!SUPPORTED.includes(network.name)) {
    console.error(`Bu script yalnızca Ethereum ağları için: ${SUPPORTED.join(", ")}`);
    console.error(`BSC için: npm run verify:bscTestnet veya npm run verify:bsc`);
    process.exitCode = 1;
    return;
  }

  if (!process.env.ETHERSCAN_API_KEY) {
    console.error("ETHERSCAN_API_KEY ayarlanmamış. .env dosyasını kontrol et.");
    process.exitCode = 1;
    return;
  }
  ...
  console.log(`Explorer : https://${network.name === "mainnet" ? "" : network.name + "."}etherscan.io`);
  ...
  if (!allPassed) {
    ...
    console.log("\n✓ Tüm Ethereum kontratları doğrulandı!");
  }
}
